async function queryInput() {
    const inputElement = document.getElementById('gameQuery'); // Seleciona o elemento de input
    const query = inputElement.value; // Obtém o valor do input

    // Verifica se 2 ou mais caracteres foram digitados
    if (query.length > 2) {
        try {
            const response = await fetch(`/tools?search=${query}`); // Faz a requisição para a rota de busca
...
document.addEventListener('DOMContentLoaded', () => {
    const gameQueryInput = document.getElementById('gameQuery');
